...
export default function Pilotos() {
  ...
  return (
    <div style={{ padding: "40px 20px", color: "#fff" }}>
      <h1 style={{ borderLeft: "5px solid #e10600", paddingLeft: "10px", marginBottom: "30px" }}>PILOTOS OFICIALES</h1>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "20px" }}>
        {pilotosGrid.map((p) => (
          <div 
            key={p.slug}
            onClick={() => navigate(`/productos?piloto=${p.slug}`)}
            style={{ backgroundColor: "#141414", border: "1px solid #222", borderRadius: "8px", padding: "20px", cursor: "pointer", position: "relative" }}
          >
            <div style={{ position: "absolute", top: "10px", right: "15px", fontSize: "2rem", opacity: 0.1, fontWeight: "bold" }}>#{p.numero}</div>
            <div style={{ fontSize: "1.5rem" }}>{p.bandera}</div>
            <h3 style={{ margin: "5px 0" }}>{p.nombre}</h3>
            <p style={{ color: "#888", fontSize: "0.9rem" }}>{p.equipo}</p>
          </div>
        ))}
      </div>
    </div>
